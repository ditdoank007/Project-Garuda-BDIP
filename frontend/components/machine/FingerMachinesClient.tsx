...
export default function FingerMachinesClient({
  ...
  const [pullTimes, setPullTimes] =
    useState<string[]>([]);

  const [pullLoading, setPullLoading] =
    useState(true);

  const [pullSaving, setPullSaving] =
    useState(false);

  const [pullMessage, setPullMessage] =
    useState("");

  const [newPullTime, setNewPullTime] =
    useState("");

  const [selectedPullTimes, setSelectedPullTimes] =
    useState<string[]>([]);
  ...
  const [manualPullMachine, setManualPullMachine] =
    useState<FingerMachine | null>(null);

  const [manualPullLoading, setManualPullLoading] =
    useState(false);

  const [manualPullResult, setManualPullResult] =
  ...
  function openManualPullModal(
    ...
    setManualPullMachine(machine);
    setManualPullLoading(false);
    setManualPullResult(null);
    setManualPullError("");
  }


  function closeManualPullModal() {
    if (manualPullLoading) {
      return;
    }
  ...
  async function executeManualPull() {
    if (!manualPullMachine) {
      return;
    }

    try {
      setManualPullLoading(true);
      setManualPullResult(null);
      setManualPullError("");

      const response =
        await fetch(
          `/api/finger-machines/${manualPullMachine.code}/runtime/manual-pull`,
          {
            method: "POST",
            cache: "no-store",
          },
  ...
  async function loadPullSchedule() {
    try {
      setPullLoading(true);

      const response =
        await getFingerMachinePullSchedule();
      ...
      setPullTimes(allTimes);

      setSelectedPullTimes(activeTimes);

    } catch (error) {
      console.error(
        "Failed loading pull schedule:",
        error,
      );
    } finally {
  ...
  function togglePullTime(
    time: string,
  ) {
    setSelectedPullTimes(
      (current) =>
  ...
  async function savePullSchedule() {
    try {
      setPullSaving(true);
      setPullMessage("");

      await updateFingerMachinePullSchedule(
        selectedPullTimes,
      );

      setPullMessage(
        "Jadwal penarikan berhasil disimpan.",
      );

      await loadPullSchedule();

    } catch (error) {
      console.error(
        "Failed saving pull schedule:",
        error,
      );

      setPullMessage(
        "Gagal menyimpan jadwal penarikan.",
      );
        ...
        <div className="flex items-start justify-between gap-6">
          ...
          <div>
            <h2 className="text-lg font-semibold">
              JADWAL PENARIKAN DATA FINGER
            </h2>

            <p className="mt-1 text-sm text-slate-500">
              Atur waktu penarikan data finger yang
